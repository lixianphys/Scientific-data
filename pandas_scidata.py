...
def pd_import(files,skiprows,usecols,names,step,Ref,AsptRatio = 3):
            ...
            for f in files:
                    ...
                    data['sxx'] = data.rxx / ((data.rxx/AsptRatio) ** 2 + data.rxy ** 2)
                    data['sxy'] = data.rxy / ((data.rxx/AsptRatio) ** 2 + data.rxy ** 2)
...
def pd_output(pd_data,WorkingFolder, filename, step):
    ...
    with open(os.path.join(WorkingFolder, filename), 'w') as outfile:
        pd_data.to_csv(outfile)

    desc = filename.strip('.cvs') + '_readme.txt'
